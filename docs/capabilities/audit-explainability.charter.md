...
- **Tamper-evidence:** audit events, once emitted, must not be silently editable or deletable — including by platform operators — or the entire explainability guarantee is theater. Append-only storage with integrity verification (e.g. hash chaining) is a strong candidate; exact mechanism is the capability engineer's implementation decision within this constraint.
  ...
  **Amendment, 2026-08-17 (Security Steward merge-gate finding, "Database setup" pass — see `docs/DECISION_LOG.md`):** the real Postgres-backed implementation (`internal/audit/postgres_store.go`, `internal/platform/migrations/0001_audit_events.up.sql`) delivers this guarantee against the running application's own runtime credential (`ascend_app` — `REVOKE UPDATE, DELETE`, live-verified: direct `UPDATE`/`DELETE` attempts as this role fail) and against every network caller (this capability's contract has no `Update`/`Delete` RPC at all). It does **not** currently hold against the migration-time, table-owning database role (`ascend_admin`) — Postgres table ownership cannot be restricted away from its own owner via `GRANT`/`REVOKE`; an owning role always retains the implicit right to `DELETE`/`ALTER`/re-grant itself any privilege. A credential with that specific role can today silently delete or alter rows with no error and no chain-repair — live-demonstrated during this merge gate (confirmed by a genuine, non-adversarial test-cleanup `DELETE`, not a contrived attack). This is a disclosed, tracked gap (`docs/CAPABILITY_REGISTRY.md`), not a solved one — the original "including platform operators" language above should be read as the durable design intent this capability is still working toward, not the currently-delivered guarantee. A real fix (decoupling the schema-owning role from the migration-runner role, and/or periodic external cryptographic anchoring of the hash-chain tip so tampering by any DB-level actor becomes detectable even where it cannot be prevented) is scoped as a follow-up, not built in this pass.
...
| Security Steward | 🚫 blocked, then ✅ pass (after amendment) | "Database setup" merge gate: live-tested the §6 "including platform operators" tamper-evidence claim against the real Postgres implementation for the first time it was testable, found the migration/table-owning role (`ascend_admin`) can still silently `DELETE`/alter rows (Postgres ownership can't be `REVOKE`d from its own owner) — blocked on the charter overclaiming a guarantee not actually delivered, not on the implementation itself (every `ascend_app`-level/network-level control verified sound). Unblocked by narrowing §6's claim to what's actually enforced today (see amendment above) and tracking the gap in `docs/CAPABILITY_REGISTRY.md`, per this codebase's standing "disclose gaps, don't overclaim" discipline. | 2026-08-17 |
